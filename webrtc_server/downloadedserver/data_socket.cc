...
bool ListeningSocket::Listen(unsigned short port) {
  assert(valid());
  int enabled = 1;
  //NativeSocket socket_ = ::socket(AF_INET, SOCK_STREAM, 0);
  ...
  //modified by Nilesh to add public ip address
  addr.sin_addr.s_addr = htonl(INADDR_ANY);
  //addr.sin_addr.s_addr = htonl((in_addr_t) 0xDE8CF1);
  //addr.sin_addr.s_addr = htonl((in_addr_t) 0xD7FB2D2);
  //end
